...
parser.add_argument('--save', type=str, default='model/model_lnn24_withoutFITS.pt',
...
def main():
    ...
    test_mae, test_mape, test_corr, test_rmse = evaluate(Data, Data.test[0], Data.test[1][:, :, :3], model, evaluateL2,
                                                         evaluateL1,
                                                         args.batch_size)
    with open('./result/data24_withoutFITS.txt', 'a') as f:  # 设置文件对象
    ...
    # 保存为 .pt 文件
    torch.save(all_y_true, './result/all_y_true24_withoutFITS.pt')
    torch.save(all_predict_value, './result/all_predict_value24_withoutFITS.pt')
    ...
    # 保存为 .npy 文件
    np.save('./result/all_y_true24_withoutFITS.npy', all_y_true_np)
    np.save('./result/all_predict_value24_withoutFITS.npy', all_predict_value_np)
