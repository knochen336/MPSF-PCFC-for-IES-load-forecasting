...
parser.add_argument('--save', type=str, default='model/model_lnn.pt',
...
parser.add_argument('--seq_out_len', type=int, default=72, help='output sequence length')
...
parser.add_argument('--epochs', type=int, default=33, help='')
...
def main():
    ...
    with open('./result/data72.txt', 'a') as f:  # 设置文件对象
    ...
    best_val = 10000000
    optim = Optim(
        model.parameters(), args.optim, args.lr, args.clip, 'min', 0.8, 4, lr_decay=args.weight_decay
    )
    ...
    try:
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            train_loss, train_mae_loss = train(Data, Data.train[0], Data.train[1][:, :, :3], model, criterion, optim,
                                               args.batch_size)
            val_mae, val_mape, val_corr ,val_rmse= evaluate(Data, Data.valid[0], Data.valid[1][:, :, :3], model, evaluateL2,
                                                   evaluateL1,
                                                   args.batch_size)
            ...
            print(
                '| end of epoch {:3d} | time: {:5.2f}s | train_mape_loss {:5.4f}| train_mae_loss {:5.4f} | valid mae {:5.4f} | valid mape {:5.4f} | valid corr  {:5.4f} | valid rmse  {:5.4f}'.format(
                    epoch, (time.time() - epoch_start_time), train_loss, train_mae_loss, val_mae, val_mape, val_corr,val_rmse),
                flush=True)
            ...
            if epoch % 1 == 0:
                ...
                with open('./result/data72.txt', 'a') as f:  # 设置文件对象
                    print(
                        "test mae {:5.4f} | test mape {:5.4f} | test corr {:5.4f} | test rmse {:5.4f}".format(test_mae, test_mape,
                                                                                          test_corr,test_rmse),
                        flush=True, file=f)

                print("test mae {:5.4f} | test mape {:5.4f} | test corr {:5.4f} | test rmse {:5.4f}".format(test_mae, test_mape, test_corr,test_rmse),
                      flush=True)
    ...
    test_mae, test_mape, test_corr,test_rmse = evaluate(Data, Data.test[0], Data.test[1][:, :, :3], model, evaluateL2, evaluateL1,
                                              args.batch_size)
    with open('./result/data72.txt', 'a') as f:  # 设置文件对象
        print("final test mae {:5.4f} | test mape {:5.4f} | test corr {:5.4f} | test rmse {:5.4f}".format(test_mae, test_mape, test_corr,test_rmse),
              file=f)
    print("final test mae {:5.4f} | test mape {:5.4f} | test corr {:5.4f} | test rmse {:5.4f}".format(test_mae, test_mape, test_corr,test_rmse))

    all_y_true, all_predict_value = plow(Data, Data.test[0], Data.test[1][:, :, :3], model, args.batch_size)
    # 保存为 .pt 文件
    torch.save(all_y_true, './result/all_y_true72.pt')
    torch.save(all_predict_value, './result/all_predict_value72.pt')
    ...
    # 保存为 .npy 文件
    np.save('./result/all_y_true72.npy', all_y_true_np)
    np.save('./result/all_predict_value72.npy', all_predict_value_np)
