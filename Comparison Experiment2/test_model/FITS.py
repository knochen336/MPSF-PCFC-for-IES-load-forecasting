import torch
import torch.nn as nn
import torch.nn.functional as F
import numpy as np

class Model(nn.Module):
    # FITS: Frequency Interpolation Time Series Forecasting
    def __init__(self, configs):
        super(Model, self).__init__()
        self.seq_len = configs.seq_len
        self.pred_len = configs.pred_len
        self.individual = configs.individual
        self.channels = configs.enc_in
        base_T = 24
        H_order = 6
        cut_freq = int(self.seq_len // base_T + 1) * H_order + 10
        self.dominance_freq = cut_freq   # 22

        self.length_ratio = (self.seq_len + self.pred_len)/self.seq_len    # 2

        if self.individual:
            self.freq_upsampler = nn.ModuleList()
            for i in range(self.channels):
                self.freq_upsampler.append(nn.Linear(self.dominance_freq, int(self.dominance_freq*self.length_ratio)).to(torch.cfloat))

        else:
            self.freq_upsampler = nn.Linear(self.dominance_freq, int(self.dominance_freq*self.length_ratio)).to(torch.cfloat) # complex layer for frequency upcampling]

    def forward(self, x, x_mark_enc=None, x_mark_dec=None, x_dec=None):
        x = x.squeeze(1).permute(0, 2, 1)

        # RIN
        x_mean = torch.mean(x, dim=1, keepdim=True)
        x = x - x_mean
        x_var = torch.var(x, dim=1, keepdim=True) + 1e-5
        x = x / torch.sqrt(x_var)

        low_specx = torch.fft.rfft(x, dim=1)
        low_specx[:, self.dominance_freq:, :] = 0  # LPF
        low_specx = low_specx[:, 0:self.dominance_freq, :]  # LPF

        # Ensure low_specxy_ is initialized
        if self.individual:
            low_specxy_ = torch.zeros(
                [low_specx.size(0), int(self.dominance_freq * self.length_ratio), low_specx.size(2)],
                dtype=low_specx.dtype).to(low_specx.device)
            for i in range(self.channels):
                low_specxy_[:, :, i] = self.freq_upsampler[i](low_specx[:, :, i].permute(0, 1)).permute(0, 1)
        else:
            low_specxy_ = self.freq_upsampler(low_specx.permute(0, 2, 1)).permute(0, 2, 1)

        # Now low_specxy_ is initialized, and we can safely use it
        low_specxy = torch.zeros(
            [low_specxy_.size(0), int((self.seq_len + self.pred_len) / 2 + 1), low_specxy_.size(2)],
            dtype=low_specxy_.dtype, device=low_specxy_.device)
        low_specxy[:, 0:low_specxy_.size(1), :] = low_specxy_  # zero padding

        low_xy = torch.fft.irfft(low_specxy, dim=1)
        low_xy = low_xy * self.length_ratio  # energy compensation for the length change

        xy = (low_xy) * torch.sqrt(x_var) + x_mean
        xy = xy[:, -self.pred_len:, :3]

        return xy

#class Config:
#    def __init__(self):
#        self.seq_len = 48      # 输入序列的长度
#        self.pred_len = 24     # 预测长度
#        self.individual = True # 是否使用个体频率插值
#        self.enc_in = 1        # 输入特征维度

#configs = Config()
#import torch

# 假设参数
#batch_size = 128
#seq_len = 48
#pred_len = 24
#channels = 1

# 模拟输入数据
#x = torch.randn(batch_size, seq_len, channels)  # 输入时间序列数据
#x_dec = torch.randn(batch_size, pred_len, channels)  # 解码器输入数据（预测）


## 初始化模型
#model = Model(configs)

# 前向传播
#output = model(x, x_mark_enc=None, x_mark_dec=None)

# 打印输出形状
#print(f"Output shape: {output.shape}")
